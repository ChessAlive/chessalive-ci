...
function startBuild() {
  ...
  const child = spawn("bash", ["./release/local-release.sh"], {
    cwd: root,
    env: { ...process.env, SOURCE_DIR: sourceDir, LOCAL_DEPLOY: "yes", PUBLIC_URL: productionUrl, SKIP_INSTALL: "no", SKIP_TESTS: "no", SKIP_WEB_SETUP: "no", SKIP_BUDGETS: "no", SKIP_ASSETS: "yes", SKIP_CONTENT: "yes", SKIP_DEPLOY: "no" },
